@section('titulo', 'Membros | Administrativo')
                    ...
                    <table class="table table-striped table-bordered">
                        ...
                        <tbody>
                            @foreach ($membros as $membro)
                                <tr>
                                    <td>{{ $membro->nome }}</td>
                                    <th>{{ ($membro->ativo == 1) ? 'Sim' : 'Não' }}</th>
                                    <th>
                                        <a href="{{ route('membro.show', ['membro' => $membro->id]) }}">
                                    ...
                                    <th>
                                        <a href="{{ route('membro.edit', ['membro' => $membro->id]) }}">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                stroke-width="1.5" stroke="currentColor" class="w-6 h-6 text-blue-600">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                    ...
                                    <th>
                                        <form action="{{ route('membro.destroy', ['membro' => $membro->id]) }}" method="POST"
