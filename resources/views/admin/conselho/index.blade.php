@section('titulo', 'Conselho | Administrativo')
    ...
    <div class="container margin40T">
        ...
        <div class="informacao-pagina">
            <div style="width:90%; margin-left: auto; margin-right:auto;">
                    ...
                    <table class="table table-striped table-bordered">
                        ...
                        <tbody>
                            @foreach ($conselhos as $conselho)
                                <tr>
                                    <td>{{ $conselho->nome }}</td>
                                    <th>{{ ($conselho->ativo == 1) ? 'Sim' : 'Não' }}</th>
                                    <th>
                                        <a href="{{ route('conselho.show', ['conselho' => $conselho->id]) }}">
                                    ...
                                    <th>
                                        <a href="{{ route('conselho.edit', ['conselho' => $conselho->id]) }}">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                stroke-width="1.5" stroke="currentColor" class="w-6 h-6 text-blue-600">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                    ...
                                    <th>
                                        <form action="{{ route('conselho.destroy', ['conselho' => $conselho->id]) }}" method="POST"
                    ...
                    {{ $conselhos->appends($request)->links() }}
            </div>
        </div>
    </div>
</x-app-layout>
